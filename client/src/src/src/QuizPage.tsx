...
export default function QuizPage() {
  ...
  const [xpAwarded, setXpAwarded] = useState(false); // Track if XP has been awarded
  const [xpAwardMessage, setXpAwardMessage] = useState(""); // Message for XP award
  ...
  // Modify the useEffect for awarding XP
  // First fix the port number in the XP award useEffect
useEffect(() => {
  const awardXP = async () => {
    if (quizCompleted && !xpAwarded && quiz) {
      // Check if score is 4+ out of 5 (80% or higher)
      ...
      if (scorePercentage >= 0.8) {
        try {
          ...
          const response = await axios.post(
            'http://localhost:3001/api/auth/add-xp', 
            { xpAmount: 50 },
            { 
              headers: {
          ...
          if (response.data.success) {
            setXpAwarded(true);
            setXpAwardMessage("🎉 Congratulations! You earned 50 XP for your excellent score! 🎉");
            console.log('XP awarded successfully:', response.data);
          }
        } catch (error) {
  ...
  awardXP();
}, [quizCompleted, xpAwarded, score, quiz]);
  ...
  // Modify handleNextQuestion to check score when quiz ends
const handleNextQuestion = () => {
  ...
  if (currentQuestion < quiz.questions.length - 1) {
    ...
    // Quiz is ending, calculate final score
    const finalScore = score;
    console.log(`Final Score: ${finalScore}/${quiz.questions.length}`);
    
    // Set quiz as completed which will trigger the XP award useEffect
          ...
          <Card className="quiz-card quiz-results">
            ...
            <CardContent>
              <div className="quiz-score-container">
                ...
                {/* Display XP award message if applicable */}
                {xpAwarded && (
                  <div className="xp-award-notification">
                    <p>{xpAwardMessage}</p>
                  </div>
                )}
              </div>
